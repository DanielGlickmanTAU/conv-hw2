...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.empty(in_features, out_features, requires_grad=True)
        torch.nn.init.normal_(self.w, std=wstd)
        self.b = torch.empty(out_features, requires_grad=True)
        torch.nn.init.normal_(self.b, std=wstd)
        # ========================
    ...
    def backward(self, dout):
        ...
        db = torch.ones(dout.shape[0]) @ dout
        self.db += db
...
class CrossEntropyLoss(Block):
    ...
    def forward(self, x, y):
        ...
        log_sum_exp = torch.exp(x).sum(dim=1).log()
